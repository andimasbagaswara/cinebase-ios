...
extension URL {
    ...
    static func urlForSearchMovie(query: String) -> URL? {
        return URL(string: "\(Constants.baseURL)/search/movie?api_key=\(Constants.apiKey)&language=en-US&query=\(query)&page=1&include_adult=false")
    }
}
